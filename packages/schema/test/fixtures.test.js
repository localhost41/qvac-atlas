...
test("all golden reports validate and derive their checked-in claims", async () => {
  ...
  for (const name of names) {
    ...
    const fixtureClaim = deriveReportClaim(report, { standardProfiles: profiles });
    assert.deepEqual(
      fixtureClaim,
      { observation: "inconclusive", claim: "unknown", actual_backend_claim: null, reasons: ["fixture-evidence"] },
      `${name} fixture isolation`,
    );
    const claim = deriveReportClaim(asProbe(report), { standardProfiles: profiles });
    assert.deepEqual(claim, expected[name], name);
    assert.equal(validateDerivedClaim(claim), true, `${name}: ${JSON.stringify(validateDerivedClaim.errors)}`);
  }
});
...
test("unknown and skipped evidence never becomes failure or success", async () => {
  ...
  for (const name of ["missing-qvac.json", "inconclusive.json"]) {
    const claim = deriveReportClaim(asProbe(await jsonFixture(name)), { standardProfiles: profiles });
    assert.equal(claim.claim, "unknown");
    assert.equal(claim.observation, "inconclusive");
  }
});

test("a generic GPU request falling back to CPU cannot claim requested-device success", async () => {
  const profiles = await standardProfiles();
  const claim = deriveReportClaim(asProbe(await jsonFixture("fallback.json")), { standardProfiles: profiles });
